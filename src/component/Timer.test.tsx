...
describe('mounted Timer', () => {
    ...
    it("should render instances of the TimerButton component", () => {
        expect(container.find("TimerButton").length).toEqual(4)
    });

    it('invokes startTimer when the start button is clicked', () => {
        const instance = container.instance();
        const spy = jest.spyOn(instance, 'startTimer');
    ...
    it('invokes stopTimer when the stop button is clicked', () => {
        const instance = container.instance();
        const spy = jest.spyOn(instance, 'stopTimer');
        container.instance().forceUpdate();
        expect(spy).toHaveBeenCalledTimes(0);
    ...
    it('invokes resetTimer when the reset button is clicked', () => {
        const instance = container.instance();
        const spy = jest.spyOn(instance, 'resetTimer');
        container.instance().forceUpdate();
        expect(spy).toHaveBeenCalledTimes(0);
